...
def simulate(
    ...
    def animate(i):
        ...
        c1_x = x - ((rob_diam/2)*cos(th))
        c1_y = y - ((rob_diam/2)*sin(th))
        c2_x = x + ((rob_diam/2)*cos(th))
        c2_y = y + ((rob_diam/2)*sin(th))
        circle1.set_center((c1_x,c1_y))
        circle2.set_center((c2_x,c2_y))
    ...
    c1_x = reference[0] - ((rob_diam/2)*cos(reference[2]))
    c1_y = reference[1] - ((rob_diam/2)*sin(reference[2]))
    c2_x = reference[0] + ((rob_diam/2)*cos(reference[2]))
    c2_y = reference[1] + ((rob_diam/2)*sin(reference[2]))
    circle1 = ptc.Circle((c1_x,c1_y), rob_diam/2, edgecolor = 'none', facecolor = 'none', linestyle="--")
    ax.add_patch(circle1)
    circle2 = ptc.Circle((c2_x,c2_y), rob_diam/2, edgecolor = 'none', facecolor = 'none', linestyle="--")
