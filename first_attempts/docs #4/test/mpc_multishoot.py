...
# The Simulation Loop
t0 = 0
init = np.array([x_init, y_init,theta_init])
state_init = ca.DM(init)  # initial state
target = np.array([x_target, y_target, theta_target])
state_target = ca.DM(target)  # target state
